...
class UserEventRepository
{
    ...
    {
        $this->db->query('SELECT e.*, u.email as user_email, u.name as user_name, c.name as category_name, c.id as category_id FROM ' . self::db_name . ' e INNER JOIN user u ON e.user_id = u.id INNER JOIN category c ON e.category_id = c.id WHERE e.' . $key . ' = :value AND e.deleted_at is null');
    ...
    //buat crud event dari user
    public function getAll(int $position, int $limit): array
    {
        $this->db->query('SELECT e.*, u.email as user_email, u.name as user_name, c.name as category_name FROM ' . self::db_name . ' e INNER JOIN user u ON e.user_id = u.id INNER JOIN category c ON e.category_id = c.id WHERE e.deleted_at is null AND user_id = :user_id ORDER BY e.id DESC LIMIT :position, :limit');
        $this->db->bind('user_id', $this->user_id);
    ...
    {
        $this->db->query('SELECT e.*, u.email as user_email, u.name as user_name, c.name as category_name FROM ' . self::db_name . ' e INNER JOIN user u ON e.user_id = u.id INNER JOIN category c ON e.category_id = c.id WHERE e.title LIKE :title AND e.deleted_at is null ORDER BY e.id DESC LIMIT :position, :limit');
    ...
    {
        $this->db->query('SELECT e.*, u.email as user_email, u.name as user_name, c.name as category_name FROM ' . self::db_name . ' e INNER JOIN user u ON e.user_id = u.id INNER JOIN category c ON e.category_id = c.id WHERE e.title LIKE :title AND e.deleted_at is null AND e.user_id != :user_id ORDER BY e.id DESC LIMIT :position, :limit');
        $this->db->bind('title', '%' . $title . '%');
        $this->db->bind('user_id', $this->user_id);
        $this->db->bind('position', $position);
        $this->db->bind('limit', $limit);
